...
export class LegacyMiningMission extends Mission {
    ...
    public static Add(operation: Operation, preferLinkMiner: boolean, localStorage = false) {
        ...
        for (let i = 0; i < operation.state.sources.length; i++) {
            ...
            operation.addMission(new LegacyMiningMission(operation, "miner" + i, source, localStorage));
        }
    }
    ...
    private paverCallback = (): PaveData => {
        ...
        let startingPosition: RoomPosition;
        if (this.state.storage) {
            startingPosition = this.state.storage.pos;
        } else {
            let spawn = this.room.find<StructureSpawn>(FIND_MY_SPAWNS)[0];
            if (spawn) {
                startingPosition = spawn.pos;
            } else {
                let site = this.room.find<ConstructionSite>(FIND_CONSTRUCTION_SITES,
                    {filter: ( (s: ConstructionSite) => s.structureType === STRUCTURE_SPAWN)})[0];
                if (site) {
                    startingPosition = site.pos;
                } else if (Game.map.getRoomLinearDistance(this.roomName, this.spawnGroup.room.name) <= 2) {
                    startingPosition = this.spawnGroup.pos;
                } else {
                    console.log(`PAVER: path too long for miner in ${this.operation.name}`);
                }
            }
        }
    ...
    private cartActions(cart: Agent) {
        ...
        // drop off energy
        let destination: Structure = this.state.storage;
        if (!destination) {
            destination = cart.findAnyEmpty(this.spawnGroup.room);
            if (!destination) {
                cart.idleOffRoad();
                return;
            }
    ...
    get analysis(): TransportAnalysis {
        if (!this._analysis) {
            let distance = this.memory.pathDistance || 50;
            this._analysis = this.cacheTransportAnalysis(distance, Mission.loadFromSource(this.state.source));
        }
