...
export class MasonAgent extends Agent {
    ...
    getRampart(): StructureRampart {
        let findRampart = () => {
            ...
            let lowestRampart = _(this.missionRoom.findStructures<StructureRampart>(STRUCTURE_RAMPART)).sortBy("hits").head();
            if (lowestRampart) {
                lowestHits = lowestRampart.hits;
            }
            let myRampart = _(this.missionRoom.findStructures<StructureRampart>(STRUCTURE_RAMPART))
    ...
    getExtension(rampart: StructureRampart): StructureExtension | StructureStorage {
        let fullExtensions = _.filter(this.missionRoom.findStructures<StructureExtension>(STRUCTURE_EXTENSION),
            (e: StructureExtension) => e.energy > 0);
        let extension = rampart.pos.findClosestByRange<StructureExtension>(fullExtensions);
        return this.creep.pos.findClosestByRange([this.missionRoom.storage, extension])
    }
